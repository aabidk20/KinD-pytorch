...
if __name__ == "__main__":
    ...
    pretrain_decom = torch.load(decom_net_dir, map_location=device)
    model.decom_net.load_state_dict(pretrain_decom)
    log('Model loaded from decom_net.pth')
    pretrain_resotre = torch.load(restore_net_dir, map_location=device)
    model.restore_net.load_state_dict(pretrain_resotre)
    log('Model loaded from restore_net.pth')
    pretrain_illum = torch.load(illum_net_dir, map_location=device)
